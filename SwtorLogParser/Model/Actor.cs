...
public class Actor
{
    ...
    public bool IsCompanion => Roms.Count > 0 && Roms[0].Span.Length > 0 && Roms[0].Span[0] == '@' &&
                               Roms[0].Span.IndexOf('/') > 0;
    ...
    private string? GetName()
    {
        ...
        {
            if (IsCompanion)
                return Roms[0].Slice(Roms[0].Span.IndexOf('/') + 1,
                    Roms[0].Span.IndexOf('{') - 1 - Roms[0].Span.IndexOf('/')).Trim().ToString();
    ...
    {
        return $"{Name} {Id} ({Health}/{MaxHealth})";
    }
    ...
    private static List<float> ExtractPosition(ReadOnlySpan<char> span)
    {
        ...
        if (float.TryParse(content.Slice(start, content.Length - start), NumberStyles.Float,
                CultureInfo.InvariantCulture, out var lastNumber)) position.Add(lastNumber);
